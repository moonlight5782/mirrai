...
export async function GET(request: Request) {
  ...
  const throttle = await rateLimitPolicy(request, { scope: "widget-config", ipLimit: 240, subjectLimit: 180, windowSeconds: 60, subject: shopSlug });
...
export async function POST(request: Request) {
  ...
  const throttle = await rateLimitPolicy(request, { scope: "widget-config-batch", ipLimit: 180, subjectLimit: 90, windowSeconds: 60, subject: shopSlug });
