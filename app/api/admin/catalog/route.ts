...
export async function GET(request: Request) {
  ...
  const items = rows.map(({ product, model }) => { let imageUrls: string[] = []; try { imageUrls = JSON.parse(product.imageUrls); } catch { /* malformed legacy value becomes empty */ } return { ...product, imageUrls: imageUrls.filter(value => typeof value === "string" && value.startsWith("https://")).slice(0, 12), model: model ?? { status: "missing", glbUrl: null, usdzUrl: null, sourceType: product.sourceUrl ? "website_photo" : "none", validationMessage: product.sourceUrl ? "Фото с сайта найдено — 3D-модель ещё не создана" : "3D-модель не загружена", qualityScore: null } }; });
