...
export async function POST(request: Request) {
  ...
  for (const product of selected) {
    ...
    if (active?.status === "requested") await db.update(generationJobs).set({ status: nextStatus, sourceImages: JSON.stringify(images), errorCode, errorMessage: message, updatedAt: new Date().toISOString() }).where(eq(generationJobs.id, active.id));
