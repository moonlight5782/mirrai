...
export async function GET(request: Request) {
  ...
  const items = rows.map(({ product, model }) => {
    const variants = (variantsByProduct.get(product.id) ?? []).map(variant => {
      const published = variant.modelStatus === "published" && Boolean(variant.glbUrl);
      return { id: String(variant.id), externalId: variant.externalId, sku: variant.sku, name: variant.name, colorName: variant.colorName, color: variant.colorHex, material: variant.material || product.material, image: variant.imageUrl, model: published ? variant.glbUrl : null, iosModel: published ? variant.usdzUrl : null, modelStatus: variant.modelStatus, default: variant.isDefault, available: published, published };
    });
    const preferred = variants.find(variant => variant.default && variant.available) ?? variants.find(variant => variant.available) ?? variants.find(variant => variant.default) ?? variants[0];
    const modelStatus = preferred?.modelStatus ?? model?.status ?? "missing";
    const published = modelStatus === "published";
    const glb = published ? (preferred?.model ?? model?.glbUrl ?? null) : null;
    const usdz = published ? (preferred?.iosModel ?? model?.usdzUrl ?? null) : null;
    return { id: product.externalId || product.sku, sku: product.sku, name: product.name, category: product.category, price: product.price, material: preferred?.material ?? product.material, color: preferred?.color ?? product.color, width: product.widthCm, height: product.heightCm, depth: product.depthCm, sourceUrl: product.sourceUrl, images: safeImages(product.imageUrls), variants, selectedVariantId: preferred?.id ?? null, modelStatus, modelMessage: model?.validationMessage ?? "3D-модель ещё не подготовлена", model: glb, iosModel: usdz, demoAvailable: published && Boolean(glb), published: published && Boolean(glb) };
  });
