...
export function SetupWizard({ displayName }: { displayName: string }) {
  ...
  async function save(event: React.FormEvent) {
    ...
    const response = await fetch("/api/admin/setup", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ websiteUrl, platform }) });
  ...
  return <main className="admin-shell setup-shell">
    <aside className="admin-sidebar"><Link href="/" className="admin-brand">MIRR<span>AI</span></Link><nav><b>ПОДКЛЮЧЕНИЕ</b><Link className="active" href="/admin/setup">Мастер установки</Link><Link href="/admin/catalog">Каталог моделей</Link><b>ПОМОЩЬ</b><a href="mailto:hello@mirrai.app">Написать нам</a></nav><div><small>Администратор</small><span>{displayName}</span></div></aside>
