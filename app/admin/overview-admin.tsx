...
export function OverviewAdmin({ displayName, shopSlug }: { displayName: string; shopSlug: string }) {
  ...
  return <main className="admin-shell"><AdminNavigation active="overview" displayName={displayName} shopSlug={shopSlug}/><section className="admin-main dashboard-overview">
    ...
    {data ? <><section className="overview-status"><article><span>Каталог</span><strong>{data.catalog.counts.ready} из {data.catalog.counts.total}</strong><small>товаров доступны покупателям в AR</small><a href={`/admin/catalog?shop=${encodeURIComponent(shopSlug)}`}>Открыть каталог →</a></article><article><span>Установка</span><strong>{installationIsConnected(data.setup.shop.installationStatus) ? "Работает" : "Требует настройки"}</strong><small>{data.setup.shop.allowedDomains.length ? data.setup.shop.allowedDomains.join(", ") : "Домен ещё не указан"}</small><a href={`/admin/setup?shop=${encodeURIComponent(shopSlug)}`}>Проверить установку →</a></article><article><span>Подписка</span><strong>{daysLeft === null ? "Не настроена" : `${daysLeft} дн.`}</strong><small>{data.subscription.shop.subscriptionStatus === "active" ? "активная подписка" : "осталось в текущем периоде"}</small><a href={`/admin/subscription?shop=${encodeURIComponent(shopSlug)}`}>Условия и срок →</a></article></section>
