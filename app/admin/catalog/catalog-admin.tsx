...
export function CatalogAdmin({ displayName }: { displayName: string }) {
  ...
  async function syncCatalog() { if (!data || syncing) return; setSyncing(true); setError(""); let offset: number | null = 0; try { while (offset !== null) { const response = await fetch("/api/admin/catalog/sync", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ shop: data.shop.slug, offset }) }); const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result.error === "source_unreachable" ? "Сайт магазина недоступен по защищённому HTTPS. Для hugge.md нужно продлить сертификат." : "Не удалось синхронизировать каталог сайта."); offset = result.nextOffset; } await load(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Не удалось синхронизировать каталог сайта."); } finally { setSyncing(false); } }
