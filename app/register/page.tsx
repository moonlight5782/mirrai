...
export default async function RegisterPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="merchant-auth"><a className="brand" href="/">MIRR<span>AI</span></a><section><div><p>ПЕРВЫЙ ШАГ</p><h1>Создайте кабинет</h1><span>После регистрации добавьте магазин, загрузите каталог и получите код виджета.</span></div><AuthForm mode="register" returnTo={returnTo}/><footer>Уже есть аккаунт? <a href={`/login?returnTo=${encodeURIComponent(returnTo)}`}>Войти</a></footer></section></main>;
}
