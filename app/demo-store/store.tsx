...
export function DemoStore() {
  ...
  return <main className="store-demo hugge-demo">
    ...
    {selected ? <section className="store-product hugge-product"><div className="store-gallery hugge-gallery">{(selectedVariant?.image || selected.images[0]) && failedImageSku !== selected.sku ? <img src={selectedVariant?.image || selected.images[0]} alt={`${selected.name}${selectedVariant ? ` — ${selectedVariant.colorName}` : ""}`} onError={() => setFailedImageSku(selected.sku)}/> : <div className="store-photo-fallback"><span>HUGGE</span><b>Фото товара недоступно</b><small>Карточка и статус модели получены из каталога</small></div>}<span className={`demo-model-status ${selected.demoAvailable ? "available" : ""}`}>{statusLabels[selected.modelStatus] ?? selected.modelStatus}</span></div><div className="store-info"><p className="store-category">{selected.category} / {selected.sku}</p><h2>{selected.name}</h2><p className="store-description">{selectedVariant?.material || selected.material || "Характеристики и материалы загружены из карточки товара HUGGE."}</p><strong className="store-price">{selected.price || "Цена по запросу"}</strong>{selected.variants.length > 0 && <div className="store-variant-picker"><small>Цвет</small><div>{selected.variants.map(variant => <button type="button" key={variant.id} className={selectedVariant?.id === variant.id ? "active" : ""} disabled={!variant.available} title={variant.available ? variant.colorName : `${variant.colorName}: 3D готовится`} aria-label={`Цвет ${variant.colorName}`} onClick={() => setSelectedVariantId(variant.id)}><i style={{ background: variant.color }}/><span>{variant.colorName}</span></button>)}</div></div>}<div className="store-specs"><span><small>Материал</small>{selectedVariant?.material || selected.material || "Уточняется"}</span><span><small>Размер</small>{dimensions(selected)}</span><span><small>3D-статус</small>{statusLabels[selected.modelStatus] ?? selected.modelStatus}</span></div><button className="store-cart">Добавить в корзину</button>{selected.demoAvailable ? <div id="mirrai-demo-slot" className="mirrai-demo-slot" key={`${selected.sku}-${selectedVariant?.id ?? "default"}`}/> : <button className="model-pending" disabled>AR появится после создания 3D-модели</button>}<p className="store-note">{selected.demoAvailable ? selected.modelMessage : "У товара есть реальная карточка и фотография, но для AR необходимо подготовить GLB-модель и проверить масштаб."}</p><div className="store-event"><i/><span>{event}</span></div>{selected.sourceUrl && <a className="source-product-link" href={selected.sourceUrl} target="_blank" rel="noreferrer">Оригинальная карточка HUGGE ↗</a>}</div></section> : <div className="demo-store-loading">Загружаем реальный каталог HUGGE…</div>}
