...
export function DemoStore() {
  ...
  return <main className="store-demo hugge-demo">
    ...
    <nav className="store-nav">
      ...
      <div className="store-actions"><button onClick={openSearch} aria-label="Открыть поиск">⌕</button><button onClick={openFavorites} className={favoritesOnly ? "active" : ""} aria-label="Открыть избранное">♡ <b>{favorites.length || ""}</b></button><button className="room-action" onClick={openRoom} aria-label="Открыть комнату с выбранной мебелью">Открыть комнату <b>{roomItems.length || "0"}</b></button><button onClick={() => setCartOpen(true)} aria-label="Открыть корзину">Корзина <b>{cart.length}</b></button></div>
    </nav>
    {mobileMenuOpen && <div className="mobile-store-menu"><p>Каталог</p>{categoryOrder.map(name => <button type="button" key={name} onClick={() => showCategory(name)}>{name}</button>)}<button type="button" onClick={openFavorites}>Избранное ({favorites.length})</button><button type="button" className="mobile-room-action" onClick={openRoom}>Открыть комнату ({roomItems.length})</button><button type="button" onClick={() => { setMobileMenuOpen(false); setCartOpen(true); }}>Корзина ({cart.length})</button></div>}
    {roomItems.length > 0 && <button type="button" className="room-floating" onClick={openRoom}><span>Комната</span><b>{roomItems.length} {roomItems.length === 1 ? "предмет" : "предмета"}</b><i>Открыть →</i></button>}
