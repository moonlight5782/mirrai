...
export function DemoStore() {
  ...
  return <main className="store-demo hugge-demo">
    ...
    {!detailOpen ? <>
      <header className="hugge-hero"><div><p>КАТАЛОГ HUGGE · В ДОПОЛНЕННОЙ РЕАЛЬНОСТИ</p><h1>Мебель, которую<br/>можно увидеть дома</h1><span>Откройте товар, рассмотрите текстурированную 3D-модель со всех сторон и поставьте её в свой интерьер в реальном масштабе.</span><button onClick={() => document.querySelector(".shop-catalog")?.scrollIntoView({ behavior: "smooth" })}>Выбрать мебель</button></div><div className="hero-product">{data?.items.find(item => item.sku === "HUGGE-89990")?.images[0] && <img src={data.items.find(item => item.sku === "HUGGE-89990")!.images[0]} alt="Кресло Alba"/>}<span>3D<br/>+ AR</span></div></header>
