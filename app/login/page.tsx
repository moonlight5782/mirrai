...
export default async function LoginPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="merchant-auth"><a className="brand" href="/">MIRR<span>AI</span></a><section><div><p>КАБИНЕТ МАГАЗИНА</p><h1>С возвращением</h1><span>Управляйте каталогом, 3D-моделями, установкой виджета и статистикой в одном месте.</span></div><AuthForm mode="login" returnTo={returnTo} googleEnabled={Boolean(googleConfiguration())} oauthError={typeof params.oauth === "string" ? params.oauth : undefined}/><footer>Нет аккаунта? <a href={`/register?returnTo=${encodeURIComponent(returnTo)}`}>Зарегистрироваться</a></footer></section></main>;
}
